...
Using algorithm to help financial market traders has been a trend in recent years.
Genetic algorithm is one of the approach to built an agent
which can suggest a buying, selling or holding decisions.
...
It is found that the chromosome representation has a critical effect to the result of optimization.
The most flexible one can achieve best performance on the training dataset,
...
To get a more clear view about their relationship with the training performance,
...
In this research, we successfully used genetic algorithm to design a trading agent on stock market.
...
Elitism works well for mitigate this problem.
On the contrary, the population of simple bit chromosomes has a tendency to be homogenized.
Higher mutation rate and survival rate is helpful to maintain the gene diversity.

Secondly, the chromosome representation dominantly decides the final outcome of the optimization.
The most flexible tuple representation can achieve the highest score on the training dataset.
Nevertheless, the overfitting problem causes a undesirable result on the testing dataset.
The float representation combined the financial domain knowledge and the flexibility,
which turn out to perform good enough on the training dataset and the best on testing dataset.

Some variants of genetic algorithm are not implemented in our research,
...
Moreover, on stock market, there are also other problems such as portfolio management and selection,
applying genetic algorithm on that field could be another interesting topic.
